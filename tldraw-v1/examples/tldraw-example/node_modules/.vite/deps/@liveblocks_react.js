...
function useRerender() {
  const [, update] = (0, import_react.useReducer)((x) => x + 1, 0);
  return update;
}
...
function createRoomContext(client) {
  ...
  function useStorage2() {
    const room = useRoom2(), [root, setState] = React.useState(null);
...
Please see https://bit.ly/3Niy5aP for details.`
    ...
    return "ok" === value.status ? value.value : (errorIf(
...
Please see https://bit.ly/3Niy5aP for details.`
  ...
  function useStorageValue(key, initialValue) {
    const room = useRoom2(), [root] = useStorage2(), rerender = useRerender(), frozenInitialValue = useInitial(initialValue);
    if (React.useEffect(() => {
      if (null == root)
        return;
      let liveValue = root.get(key);
      null == liveValue && (liveValue = frozenInitialValue, root.set(key, liveValue));
    ...
    }, [root, room, key, frozenInitialValue, rerender]), null == root)
      return { status: "loading" };
    {
      const value = root.get(key);
      return null == value ? { status: "notfound" } : { status: "ok", value };
    }
  }
  return {
    RoomProvider: function(props) {
      ...
      const _client = useClient$1(), frozen = useInitial({
        initialPresence,
        initialStorage,
        defaultPresence,
        defaultStorageRoot
    ...
    useErrorListener: function(callback) {
      const room = useRoom2(), savedCallback = React.useRef(callback);
      React.useEffect(() => {
        savedCallback.current = callback;
      }), React.useEffect(() => {
        const unsubscribe = room.subscribe(
          "error",
          (e) => savedCallback.current(e)
        );
        return () => {
          unsubscribe();
        };
